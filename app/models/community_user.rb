class CommunityUser < ApplicationRecord
  ...
  def ensure_maximum_join_capacity
    ...
    if !@community_detail.max_join_number || ((@community_detail.max_join_number) - (CommunityUser.where(community_id: @community_id).count) >= 1)
      ensure_limit_of_sex
    end
  end
  ...
  def ensure_limit_of_sex
    ...
    if !@community_detail.sex_limit || (@community_detail.sex_limit_before_type_cast == (@user.sex_before_type_cast - 1))
  ...
  def ensure_limit_of_age
    ...
    if !@community_detail.age_min_limit && !@community_detail.age_max_limit
      self.save
    # コミュニティの年齢制限が設定されているが、自分の年齢が範囲内にふくまれていれば加入
    else
      @community_detail.age_min_limit = 0 unless @community_detail.age_min_limit
      @community_detail.age_max_limit = 101 unless @community_detail.age_max_limit
